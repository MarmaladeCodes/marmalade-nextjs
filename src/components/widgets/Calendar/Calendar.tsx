...
function buildAndSortEvents({
	...
	events.forEach((event: VEvent, index: number): void => {
		const date = formatDate({ date: event.start.date })
		if (!date) return
		const comparedDates = compareDate(DateTime.fromFormat(date, DateFormatType.DATE))
		if (!all && comparedDates && ((past && comparedDates >= 0) || (!past && comparedDates < 0))) {
			return
		}
		eventObj[date] = <Event data={event} index={index} />
	})
	const sortedEvents = Object.keys(eventObj).sort()
	if (order === 'desc') {
		sortedEvents.reverse()
	}
	return sortedEvents.map((date) => eventObj[date])
}
