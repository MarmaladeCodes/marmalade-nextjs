...
async function fetchAPI(query = '', { variables }: Record<string, unknown> = {}) {
	...
	if (json.errors) {
		console.error('Failed to fetch API', json.errors)
...
export async function getAllPostsForHome(preview: Preview) {
	const data = await fetchAPI(
		`
    query AllPosts {
...
export async function getPostAndMorePosts({
	...
	const postPreview =
		preview && (previewData as { post: Post }).post
			? (previewData as { post: Post }).post
			: undefined
